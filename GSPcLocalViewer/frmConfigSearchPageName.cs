...
// Type: GSPcLocalViewer.frmConfigSearchPageName
...
{
  public class frmConfigSearchPageName : Form
  {
    ...
    public frmConfigSearchPageName(frmConfig frm, int _serverId)
    {
    ...
    private void InitializeSearchGrid()
    {
      ...
      {
        this.dgViewSearchSettings.Invoke((Delegate) new frmConfigSearchPageName.InitializeSearchGridDelegate(this.InitializeSearchGrid));
      }
      else
      {
        try
      ...
      try
      {
        ...
        if (Settings.Default.appLanguage.ToUpper() != "ENGLISH")
        {
          for (int index = 0; index < this.dgViewSearchSettings.Rows.Count; ++index)
            Global.SaveToLanguageIni("PAGENAME_SEARCH", this.dgViewSearchSettings[1, index].Value.ToString(), this.dgViewSearchSettings[2, index].Value.ToString(), this.serverId);
        }
        for (int index = 0; index < this.dgViewSearchSettings.Rows.Count; ++index)
        {
          string str;
          if (Settings.Default.appLanguage.ToUpper() != "ENGLISH")
            str = Global.GetEngHeaderVal("PAGENAME_SEARCH", this.dgViewSearchSettings[1, index].Value.ToString(), this.serverId).Split('|')[1];
          else
            str = this.dgViewSearchSettings[2, index].Value.ToString();
          Global.SaveToServerIni("PAGENAME_SEARCH", this.dgViewSearchSettings[1, index].Value.ToString(), this.dgViewSearchSettings[0, index].Value.ToString() + "|" + str + "|" + this.dgViewSearchSettings[4, index].Value.ToString() + "|" + this.dgViewSearchSettings[3, index].Value.ToString(), this.serverId);
        }
      }
      catch
      {
    ...
    private void AddSearchSettingsRow(string sVal, string sKey)
    {
      try
      {
        ...
        if (Settings.Default.appLanguage.ToUpper() != "ENGLISH")
          gridViewTextBoxCell2.Value = (object) Global.GetDGHeaderCellValue("PAGENAME_SEARCH", sKey, strArray1[1], this.serverId);
        else
          gridViewTextBoxCell2.Value = (object) strArray1[1];
    ...
    private void LoadDataGridView()
    {
      if (this.dgViewSearchSettings.InvokeRequired)
      {
        this.dgViewSearchSettings.Invoke((Delegate) new frmConfigSearchPageName.LoadDataGridViewXmlDelegate(this.LoadDataGridView));
      }
      else
      {
        ...
        ArrayList keys = iniFileIo.GetKeys(Application.StartupPath + "\\GSP_" + Program.iniServers[this.serverId].sIniKey + ".ini", "PAGENAME_SEARCH");
        for (int index = 0; index < keys.Count; ++index)
        {
          try
          {
            this.AddSearchSettingsRow(new IniFileIO().GetKeyValue("PAGENAME_SEARCH", keys[index].ToString(), Application.StartupPath + "\\GSP_" + Program.iniServers[this.serverId].sIniKey + ".ini"), keys[index].ToString());
          }
          catch
          {
          }
        ...
        this.AddSearchSettingsRow("true|Picture File|C|100", "PictureFile");
      }
    }
    ...
    private string GetResource(string sDefaultValue, string sKey, ResourceType rType)
    {
      try
      {
        string str = "" + "/Screen[@Name='CONFIGURATION']" + "/Screen[@Name='PAGE_NAME']";
        switch (rType)
        {
    ...
    private void InitializeComponent()
    {
      ...
      this.dataGridViewCheckBoxColumn1.AutoSizeMode = DataGridViewAutoSizeColumnMode.ColumnHeader;
      ...
      this.dataGridViewTextBoxColumn3.HeaderText = "Col Width";
      this.dataGridViewTextBoxColumn3.Name = "dataGridViewTextBoxColumn3";
      this.dataGridViewComboBoxColumn1.HeaderText = "Alignment";
      this.dataGridViewComboBoxColumn1.Items.AddRange((object) "L", (object) "R", (object) "C");
      ...
      this.Name = nameof (frmConfigSearchPageName);
