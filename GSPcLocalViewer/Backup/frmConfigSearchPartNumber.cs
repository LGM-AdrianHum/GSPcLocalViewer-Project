...
// Type: GSPcLocalViewer.frmConfigSearchPartNumber
...
namespace GSPcLocalViewer
{
  public class frmConfigSearchPartNumber : Form
  {
    ...
    private void InitializeComponent()
    {
      ...
      this.Name = nameof (frmConfigSearchPartNumber);
    ...
    private void InitializeSearchGrid()
    {
      ...
      {
        this.dgViewSearchSettings.Invoke((Delegate) new frmConfigSearchPartNumber.InitializeSearchGridDelegate(this.InitializeSearchGrid));
      }
      else
      {
        try
      ...
      try
      {
        ...
        if (Settings.Default.appLanguage.ToUpper() != "ENGLISH")
        {
          for (int index = 0; index < this.dgViewSearchSettings.Rows.Count; ++index)
            Global.SaveToLanguageIni("PARTNUMBER_SEARCH", this.dgViewSearchSettings[1, index].Value.ToString(), this.dgViewSearchSettings[2, index].Value.ToString(), this.serverId);
        }
        for (int index = 0; index < this.dgViewSearchSettings.Rows.Count; ++index)
        {
          string str;
          if (Settings.Default.appLanguage.ToUpper() != "ENGLISH")
            str = Global.GetEngHeaderVal("PARTNUMBER_SEARCH", this.dgViewSearchSettings[1, index].Value.ToString(), this.serverId).Split('|')[1];
          else
            str = this.dgViewSearchSettings[2, index].Value.ToString();
          Global.SaveToServerIni("PARTNUMBER_SEARCH", this.dgViewSearchSettings[1, index].Value.ToString(), this.dgViewSearchSettings[0, index].Value.ToString() + "|" + str + "|" + this.dgViewSearchSettings[4, index].Value.ToString() + "|" + this.dgViewSearchSettings[3, index].Value.ToString(), this.serverId);
        }
      }
      catch
      {
    ...
    private void AddSearchSettingsRow(string sVal, string sKey)
    {
      try
      {
        ...
        gridViewTextBoxCell2.Value = (object) Global.GetDGHeaderCellValue("PARTNUMBER_SEARCH", sKey, strArray1[1], this.serverId);
    ...
    private void LoadDataGridView()
    {
      if (this.dgViewSearchSettings.InvokeRequired)
      {
        this.dgViewSearchSettings.Invoke((Delegate) new frmConfigSearchPartNumber.LoadDataGridViewXmlDelegate(this.LoadDataGridView));
      }
      else
      {
        ...
        ArrayList keys = iniFileIo.GetKeys(Application.StartupPath + "\\GSP_" + Program.iniServers[this.serverId].sIniKey + ".ini", "PARTNUMBER_SEARCH");
        for (int index = 0; index < keys.Count; ++index)
        {
          try
          {
            this.AddSearchSettingsRow(new IniFileIO().GetKeyValue("PARTNUMBER_SEARCH", keys[index].ToString(), Application.StartupPath + "\\GSP_" + Program.iniServers[this.serverId].sIniKey + ".ini"), keys[index].ToString());
          }
          catch
          {
          }
    ...
    private string GetResource(string sDefaultValue, string sKey, ResourceType rType)
    {
      try
      {
        string str = "" + "/Screen[@Name='CONFIGURATION']" + "/Screen[@Name='PART_NUMBER']";
        switch (rType)
        {
