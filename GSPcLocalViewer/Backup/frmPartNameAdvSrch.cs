...
// Type: GSPcLocalViewer.frmPartNameAdvSrch
...
{
  public class frmPartNameAdvSrch : Form
  {
    ...
    public frmPartNameAdvSrch(frmOpenBook frm)
    {
      ...
      this.cmbServers.Items.Add((object) new Global.ComboBoxItem(this.GetResource("Select a server…", "SELECT_A_SERVER", ResourceType.LABEL), (object) null));
    ...
    private void btnSearch_Click(object sender, EventArgs e)
    {
      ...
      if (!result)
      {
        ...
        if (this.ltbKeyword._Text.Trim() != string.Empty)
        {
          ...
          this.ShowResultMessage(this.GetResource("Enter part name to search", "ENTER_PARTNAME", ResourceType.LABEL));
      }
      else
      {
        if (!this.thGetResults.IsAlive)
    ...
    private void dgvSearchResults_CellMouseDoubleClick(object sender, DataGridViewCellMouseEventArgs e)
    {
      ...
      try
      {
        ...
        {
          empty7 = tag[(object) "BookCode"].ToString();
          if (this.dgvSearchResults.Columns.Contains(empty7))
            empty2 = this.dgvSearchResults.Rows[e.RowIndex].Cells[empty7].Value.ToString();
        }
        if (tag.Contains((object) "PageId"))
        {
          empty7 = tag[(object) "PageId"].ToString();
          if (this.dgvSearchResults.Columns.Contains(empty7))
            empty3 = this.dgvSearchResults.Rows[e.RowIndex].Cells[empty7].Value.ToString();
        }
        if (tag.Contains((object) "PicIndex"))
        {
          empty7 = tag[(object) "PicIndex"].ToString();
          if (this.dgvSearchResults.Columns.Contains(empty7))
            empty4 = this.dgvSearchResults.Rows[e.RowIndex].Cells[empty7].Value.ToString();
        }
        if (tag.Contains((object) "ListIndex"))
        {
          empty7 = tag[(object) "ListIndex"].ToString();
          if (this.dgvSearchResults.Columns.Contains(empty7))
            empty5 = this.dgvSearchResults.Rows[e.RowIndex].Cells[empty7].Value.ToString();
        }
        if (tag.Contains((object) "PartNumber"))
        {
          empty7 = tag[(object) "PartNumber"].ToString();
          if (this.dgvSearchResults.Columns.Contains(empty7))
            empty6 = this.dgvSearchResults.Rows[e.RowIndex].Cells[empty7].Value.ToString();
        }
        if (sIniKey != string.Empty && empty2 != string.Empty && (empty3 != string.Empty && empty4 != string.Empty) && (empty5 != string.Empty && empty6 != string.Empty && empty7 != string.Empty))
        {
          this.frmParent.CloseAndLoadSearch(sIniKey, empty2, empty3, empty4, empty5, empty6);
        }
        else
        {
    ...
    private void EnableDisableSearchControls(bool value)
    {
      ...
      {
        this.pnlSearchCriteria.Invoke((Delegate) new frmPartNameAdvSrch.EnableDisableSearchControlsDelegate(this.EnableDisableSearchControls), (object) value);
      }
      else
      {
        if (this.cmbServers.Items.Count != 2)
    ...
    private void ShowLoading(Panel parentPanel)
    {
      try
      {
        ...
        {
          parentPanel.Invoke((Delegate) new frmPartNameAdvSrch.ShowLoadingDelegate(this.ShowLoading), (object) parentPanel);
        }
        else
        {
    ...
    private void HideLoading(Panel parentPanel)
    {
      try
      {
        if (parentPanel.InvokeRequired)
        {
          parentPanel.Invoke((Delegate) new frmPartNameAdvSrch.HideLoadingDelegate(this.HideLoading), (object) parentPanel);
        }
        else
        {
          foreach (Control control in (ArrangedElementCollection) parentPanel.Controls)
    ...
    private void UpdateStatus()
    {
      ...
      if (this.frmParent.InvokeRequired)
        this.frmParent.Invoke((Delegate) new frmPartNameAdvSrch.StatusDelegate(this.frmParent.UpdateStatus), (object) this.statusText);
      else
    ...
    private void AddRowToBooksGrid()
    {
      ...
      {
        this.dgvBooks.Invoke((Delegate) new frmPartNameAdvSrch.AddRowToBooksGridDelegate(this.AddRowToBooksGrid));
      }
      else
      {
    ...
    private void AddRowToResultsGrid()
    {
      ...
      {
        this.dgvSearchResults.Invoke((Delegate) new frmPartNameAdvSrch.AddRowToResultsGridDelegate(this.AddRowToResultsGrid));
      }
      else
      {
    ...
    private void ChangeSearchButtonText(string caption, bool isSearching)
    {
      ...
      {
        this.btnSearch.Invoke((Delegate) new frmPartNameAdvSrch.ChangeSearchButtonTextDelegate(this.ChangeSearchButtonText), (object) caption, (object) isSearching);
      }
      else
      {
    ...
    private void ShowResultMessage(string msg)
    {
      ...
      {
        this.pnlrtbNoDetails.Invoke((Delegate) new frmPartNameAdvSrch.ShowResultMessageDelegate(this.ShowResultMessage), (object) msg);
      }
      else
      {
    ...
    private void ShowResultGrid()
    {
      if (this.pnlSearchResults.InvokeRequired)
        this.pnlSearchResults.Invoke((Delegate) new frmPartNameAdvSrch.ShowResultGridDelegate(this.ShowResultGrid));
      else
    ...
    private void InitializeResultsGrid(XmlNode xPageSchema, XmlNode xPartSchema)
    {
      if (this.dgvSearchResults.InvokeRequired)
      {
        this.dgvSearchResults.Invoke((Delegate) new frmPartNameAdvSrch.InitializeResultsGridDelegate(this.InitializeResultsGrid), (object) xPageSchema, (object) xPartSchema);
      }
      else
      {
        try
        {
          ...
          ArrayList keys = iniFileIo.GetKeys(Application.StartupPath + "\\GSP_" + this.sServerKey + ".ini", "BOOKS_PARTNAME_ADVSEARCH");
          List<DataGridViewTextBoxColumn> viewTextBoxColumnList = new List<DataGridViewTextBoxColumn>();
          if (keys.Count > 0)
          {
            try
            {
              foreach (XmlAttribute attribute in (XmlNamedNodeMap) xPartSchema.Attributes)
              {
                if (attribute.Name != null && attribute.Value != null && (attribute.Name.Trim() != string.Empty && attribute.Value.Trim() != string.Empty))
                {
                  ...
                  viewTextBoxColumn.Name = "PART::" + attribute.Name.ToUpper();
                  ...
                  for (int index = 0; index < keys.Count; ++index)
                  {
                    ...
                    {
                      string[] strArray = iniFileIo.GetKeyValue("BOOKS_PARTNAME_ADVSEARCH", keys[index].ToString(), Application.StartupPath + "\\GSP_" + this.sServerKey + ".ini").Split('|');
                      try
                      {
                      ...
                      try
                      {
                        if (Settings.Default.appLanguage.ToUpper() != "ENGLISH")
                          viewTextBoxColumn.HeaderText = Global.GetDGHeaderCellValue("BOOKS_PARTNAME_ADVSEARCH", upper, strArray[1], this.p_ServerId);
                        else
                  ...
                  if (upper == "PARTNUMBER")
                    hashtable.Add((object) "PartNumber", (object) ("PART::" + attribute.Name.ToUpper()));
                }
              }
                ...
                if (attribute.Name != null && attribute.Value != null && (attribute.Name.Trim() != string.Empty && attribute.Value.Trim() != string.Empty))
                {
                  ...
                  for (int index = 0; index < keys.Count; ++index)
                  {
                    if (upper == keys[index].ToString().Trim())
                    {
                      ...
                      {
                        if (Settings.Default.appLanguage.ToUpper() != "ENGLISH")
                          viewTextBoxColumn.HeaderText = Global.GetDGHeaderCellValue("BOOKS_PARTNAME_ADVSEARCH", upper, strArray[1], this.frmParent.frmParent.ServerId);
                        else
                          viewTextBoxColumn.HeaderText = strArray[1];
                      }
                      catch (Exception ex)
                  ...
                  {
                    if (dataGridViewColumn.Tag.ToString().ToUpper() != string.Empty && dataGridViewColumn.Visible && dataGridViewColumn.Tag.ToString().ToUpper() == viewTextBoxColumn.Tag.ToString().ToUpper())
                    {
                      flag = true;
                      break;
                    }
                ...
                if (booksColumns.Count > 0)
                {
                  foreach (DataGridViewColumn dataGridViewColumn in (BaseCollection) booksColumns)
                  {
                    if (dataGridViewColumn.Name.ToUpper() != "CHK")
                    {
                      ...
                      viewTextBoxColumn.HeaderText = this.GetGridLanguage(dataGridViewColumn.HeaderText, this.sServerKey);
                      viewTextBoxColumn.AutoSizeMode = DataGridViewAutoSizeColumnMode.None;
                      viewTextBoxColumn.Width = 80;
                      string upper = dataGridViewColumn.Tag.ToString().ToUpper();
                      viewTextBoxColumn.Visible = false;
                      for (int index = 0; index < keys.Count; ++index)
                      {
                        if (upper == keys[index].ToString().Trim())
                      ...
                      {
                        if (column.Tag.ToString() != string.Empty && column.Visible && column.Tag.ToString().ToUpper() == viewTextBoxColumn.Tag.ToString().ToUpper())
                        {
                          flag = true;
                          break;
                        }
    ...
    private void InitializeBooksGrid(XmlNode xSchemaNode)
    {
      if (this.dgvBooks.InvokeRequired)
      {
        this.dgvBooks.Invoke((Delegate) new frmPartNameAdvSrch.InitializeBooksGridDelegate(this.InitializeBooksGrid), (object) xSchemaNode);
      }
      else
      {
        try
...
label_14:
    ...
    private string GetKeyword()
    {
      if (this.ltbKeyword.InvokeRequired)
        return (string) this.ltbKeyword.Invoke((Delegate) new frmPartNameAdvSrch.GetKeywordDelegate(this.GetKeyword));
    ...
    private bool GetMatchCase()
    {
      if (this.chkMatchCase.InvokeRequired)
        return (bool) this.chkMatchCase.Invoke((Delegate) new frmPartNameAdvSrch.GetMatchCaseDelegate(this.GetMatchCase));
    ...
    private bool GetExactMatch()
    {
      if (this.chkExactMatch.InvokeRequired)
        return (bool) this.chkExactMatch.Invoke((Delegate) new frmPartNameAdvSrch.GetExaceMatchDelegate(this.GetExactMatch));
    ...
    {
      if (this.dgvBooks.InvokeRequired)
        return (DataGridViewColumnCollection) this.dgvBooks.Invoke((Delegate) new frmPartNameAdvSrch.GetBooksColumnsDelegate(this.GetBooksColumns));
    ...
    {
      if (this.dgvBooks.InvokeRequired)
        return (ArrayList) this.dgvBooks.Invoke((Delegate) new frmPartNameAdvSrch.GetSelectedBookRowsDelegate(this.GetSelectedBookRows));
      ArrayList arrayList = new ArrayList();
      foreach (DataGridViewRow row in (IEnumerable) this.dgvBooks.Rows)
      {
        bool result = false;
    ...
    private bool LoadBooksInGrid(string sFilePath)
    {
      ...
      {
        if ((!(index3 != "") || filterBooks.Attributes[index3] == null || !(filterBooks.Attributes[index3].Value.ToUpper().Trim() == "GSC")) && (filterBooks.Attributes[index1] != null && filterBooks.Attributes[index2] != null))
        {
    ...
    private void GetResults(object arguments)
    {
      ...
      try
      {
        ...
        XmlNode xPartSchema = search.SearchPartSchema(dataGridViewRow1.Tag.ToString(), Program.iniServers[this.p_ServerId].sIniKey);
        ...
        if (xPartSchema == null && selectedBookRows.Count > 1)
        {
          DataGridViewRow dataGridViewRow2 = (DataGridViewRow) selectedBookRows[1];
          xPartSchema = search.SearchPartSchema(dataGridViewRow2.Tag.ToString(), Program.iniServers[this.p_ServerId].sIniKey);
        }
        if (xPageSchema == null || xPageSchema.Attributes.Count == 0 || (xPartSchema == null || xPartSchema.Attributes.Count == 0))
        {
          exception.HelpLink = this.GetResource("File schema not found on server", "NO_SCHEMA", ResourceType.STATUS_MESSAGE);
          throw exception;
        }
        this.InitializeResultsGrid(xPageSchema, xPartSchema);
        this.ShowResultGrid();
        foreach (DataGridViewRow dataGridViewRow2 in selectedBookRows)
        {
          XmlNode xmlNode = search.SearchParts(keyword, FieldToSearch, (matchCase ? 1 : 0) != 0, (exactMatch ? 1 : 0) != 0, Program.iniServers[this.p_ServerId].sIniKey, new string[1]
          {
          ...
          if (xmlNode != null && xmlNode.ChildNodes.Count > 0)
          {
            foreach (XmlNode childNode1 in xmlNode.ChildNodes)
            {
              ...
              if (childNode1.ChildNodes.Count > 0)
              {
                ...
                {
                  foreach (XmlAttribute attribute in (XmlNamedNodeMap) childNode2.ChildNodes[0].Attributes)
                  {
                    try
                    {
                      this.dgvSearchResults.Rows[this.dgvSearchResults.Rows.Count - 1].Cells["PART::" + attribute.Name].Value = (object) attribute.Value;
                    }
                    catch
                    {
                    }
    ...
    public void LoadResources()
    {
      ...
      this.chkMatchCase.Text = this.GetResource("Match Case", "MATCH_CASE", ResourceType.CHECKBOX);
      this.chkExactMatch.Text = this.GetResource("Exact Match", "EXACT_MATCH", ResourceType.CHECKBOX);
    ...
    private string GetResource(string sDefaultValue, string sKey, ResourceType rType)
    {
      try
      {
        string str = "" + "/Screen[@Name='OPEN_BOOKS']" + "/Screen[@Name='PART_NAME_ADVSEARCH']";
        switch (rType)
        {
          ...
          case ResourceType.CHECK_BOX:
            ...
            break;
          case ResourceType.CHECKBOX:
            str += "/Resources[@Name='CHECKBOX']";
            break;
    ...
    private void InitializeComponent()
    {
      ...
      this.Name = nameof (frmPartNameAdvSrch);
