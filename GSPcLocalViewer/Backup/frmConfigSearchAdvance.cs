...
// Type: GSPcLocalViewer.frmConfigSearchAdvance
...
{
  public class frmConfigSearchAdvance : Form
  {
    ...
    public frmConfigSearchAdvance(frmConfig frm, int _serverId)
    {
    ...
    private void InitializeSearchGrid()
    {
      ...
      {
        this.dgViewSearchSettings.Invoke((Delegate) new frmConfigSearchAdvance.InitializeSearchGridDelegate(this.InitializeSearchGrid));
      }
      else
      {
        try
      ...
      try
      {
        ...
        if (Settings.Default.appLanguage.ToUpper() != "ENGLISH")
        {
          for (int index = 0; index < this.dgViewSearchSettings.Rows.Count; ++index)
            Global.SaveToLanguageIni("ADVANCE_SEARCH", this.dgViewSearchSettings[1, index].Value.ToString(), this.dgViewSearchSettings[2, index].Value.ToString(), this.serverId);
        }
        for (int index = 0; index < this.dgViewSearchSettings.Rows.Count; ++index)
        {
          string str;
          if (Settings.Default.appLanguage.ToUpper() != "ENGLISH")
            str = Global.GetEngHeaderVal("ADVANCE_SEARCH", this.dgViewSearchSettings[1, index].Value.ToString(), this.serverId).Split('|')[1];
          else
            str = this.dgViewSearchSettings[2, index].Value.ToString();
          Global.SaveToServerIni("ADVANCE_SEARCH", this.dgViewSearchSettings[1, index].Value.ToString(), this.dgViewSearchSettings[0, index].Value.ToString() + "|" + str + "|" + this.dgViewSearchSettings[4, index].Value.ToString() + "|" + this.dgViewSearchSettings[3, index].Value.ToString(), this.serverId);
        }
      }
      catch
      {
    ...
    private void AddSearchSettingsRow(string sVal, string sKey)
    {
      try
      {
        ...
        if (Settings.Default.appLanguage.ToUpper() != "ENGLISH")
          gridViewTextBoxCell2.Value = (object) Global.GetDGHeaderCellValue("ADVANCE_SEARCH", sKey, strArray1[1], this.serverId);
        else
    ...
    private void LoadDataGridView()
    {
      if (this.dgViewSearchSettings.InvokeRequired)
      {
        this.dgViewSearchSettings.Invoke((Delegate) new frmConfigSearchAdvance.LoadDataGridViewXmlDelegate(this.LoadDataGridView));
      }
      else
      {
        ...
        ArrayList keys = iniFileIo.GetKeys(Application.StartupPath + "\\GSP_" + Program.iniServers[this.serverId].sIniKey + ".ini", "ADVANCE_SEARCH");
        for (int index = 0; index < keys.Count; ++index)
        {
          try
          {
            this.AddSearchSettingsRow(new IniFileIO().GetKeyValue("ADVANCE_SEARCH", keys[index].ToString(), Application.StartupPath + "\\GSP_" + Program.iniServers[this.serverId].sIniKey + ".ini"), keys[index].ToString());
          }
          catch
          {
          }
    ...
    private string GetResource(string sDefaultValue, string sKey, ResourceType rType)
    {
      try
      {
        string str = "" + "/Screen[@Name='CONFIGURATION']" + "/Screen[@Name='ADVANCED_SEARCH']";
        switch (rType)
        {
    ...
    private void InitializeComponent()
    {
      ...
      this.Name = "frmConfigSearchAdvancer";
