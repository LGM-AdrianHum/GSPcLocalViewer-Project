...
// Type: GSPcLocalViewer.frmPartNumberSearch
...
namespace GSPcLocalViewer
{
  public class frmPartNumberSearch : Form
  {
    ...
    public frmPartNumberSearch(frmViewer frm, frmSearch frmSearch)
    {
    ...
    private void InitializeSearchGrid()
    {
      ...
      {
        this.dgViewSearch.Invoke((Delegate) new frmPartNumberSearch.InitializeSearchGridDelegate(this.InitializeSearchGrid));
      }
      else
      {
        try
    ...
    private void LoadDataGridView()
    {
      if (this.dgViewSearch.InvokeRequired)
      {
        this.dgViewSearch.Invoke((Delegate) new frmPartNumberSearch.LoadDataGridViewXmlDelegate(this.LoadDataGridView));
      }
      else
      {
        ...
        ArrayList keys = iniFileIo.GetKeys(Application.StartupPath + "\\GSP_" + Program.iniServers[this.frmParent.ServerId].sIniKey + ".ini", "PARTNUMBER_SEARCH");
        for (int index = 0; index < keys.Count; ++index)
        {
          try
          {
            Global.AddSearchCol(new IniFileIO().GetKeyValue("PARTNUMBER_SEARCH", keys[index].ToString(), Application.StartupPath + "\\GSP_" + Program.iniServers[this.frmParent.ServerId].sIniKey + ".ini"), keys[index].ToString(), "PARTNUMBER_SEARCH", this.frmParent.ServerId, ref this.dgViewSearch);
          }
          catch
          {
          }
    ...
    private void ShowLoading(Panel parentPanel)
    {
      try
      {
        ...
        {
          this.Invoke((Delegate) new frmPartNumberSearch.ShowLoadingDelegate(this.ShowLoading), (object) this.pnlForm);
        }
        else
        {
    ...
    private void HideLoading(Panel parentPanel)
    {
      try
      {
        if (this.pnlForm.InvokeRequired)
        {
          this.Invoke((Delegate) new frmPartNumberSearch.HideLoadingDelegate(this.HideLoading), (object) this.pnlForm);
        }
        else
        {
          foreach (Control control in (ArrangedElementCollection) parentPanel.Controls)
    ...
    {
      if (this.dgViewSearch.InvokeRequired)
        this.dgViewSearch.Invoke((Delegate) new frmPartNumberSearch.GridViewClearRowsDelegate(this.GridViewClearRows));
      else
    ...
    {
      if (this.dgViewSearch.InvokeRequired)
        this.dgViewSearch.Invoke((Delegate) new frmPartNumberSearch.GridViewAddRowDelegate(this.GridViewAddRow), (object) tempRow);
      else
    ...
    private void UpdadetControls()
    {
      ...
      {
        if (this.pnlForm.InvokeRequired)
          this.Invoke((Delegate) new frmPartNumberSearch.UpdadetControlsDelegate(this.UpdadetControls));
        else
    ...
    private string GetResource(string sDefaultValue, string sKey, ResourceType rType)
    {
      try
      {
        string str = "" + "/Screen[@Name='SEARCH']" + "/Screen[@Name='PART_NUMBER_SEARCH']";
        switch (rType)
        {
      ...
      try
      {
        ...
        if (this.checkBoxMatchCase.Checked)
        {
          if (this.checkBoxExactMatch.Checked)
          {
            ...
            {
              string xpath = "//Part[@" + this.attPartNumberElement + "='" + strArray[0] + "' or @" + this.attPartNumberElement + "='" + strArray[1] + "']";
              xNodeList = xmlDocument.SelectNodes(xpath);
            }
            else
              xNodeList = xmlDocument.SelectNodes("//Part[@" + this.attPartNumberElement + "='" + this.searchString + "']");
          }
          else if (hankakuZenkakuFlag && strArray.Length == 2)
          {
            string xpath = "//Part[contains(@" + this.attPartNumberElement + ",'" + strArray[0] + "') or contains(@" + this.attPartNumberElement + ",'" + strArray[1] + "')]";
            xNodeList = xmlDocument.SelectNodes(xpath);
          }
          else
            xNodeList = xmlDocument.SelectNodes("//Part[contains(@" + this.attPartNumberElement + ",'" + this.searchString + "')]");
        }
        else if (this.checkBoxExactMatch.Checked)
        {
          if (hankakuZenkakuFlag && strArray.Length == 2)
          {
            string xpath = "//Part[translate(@" + this.attPartNumberElement + ",'abcdefghijklmnopqrstuvwxyz','ABCDEFGHIJKLMNOPQRSTUVWXYZ')='" + strArray[0].ToUpper() + "' or translate(@" + this.attPartNumberElement + ",'abcdefghijklmnopqrstuvwxyz','ABCDEFGHIJKLMNOPQRSTUVWXYZ')='" + strArray[1].ToUpper() + "']";
            xNodeList = xmlDocument.SelectNodes(xpath);
          }
          else
            xNodeList = xmlDocument.SelectNodes("//Part[translate(@" + this.attPartNumberElement + ",'abcdefghijklmnopqrstuvwxyz','ABCDEFGHIJKLMNOPQRSTUVWXYZ')='" + this.searchString.ToUpper() + "']");
        }
        else if (hankakuZenkakuFlag && strArray.Length == 2)
        {
          string xpath = "//Part[contains(translate(@" + this.attPartNumberElement + ",'abcdefghijklmnopqrstuvwxyz','ABCDEFGHIJKLMNOPQRSTUVWXYZ'),'" + strArray[0].ToUpper() + "') or contains(translate(@" + this.attPartNumberElement + ",'abcdefghijklmnopqrstuvwxyz','ABCDEFGHIJKLMNOPQRSTUVWXYZ'),'" + strArray[1].ToUpper() + "')]";
          xNodeList = xmlDocument.SelectNodes(xpath);
        }
        else
          xNodeList = xmlDocument.SelectNodes("//Part[contains(translate(@" + this.attPartNumberElement + ",'abcdefghijklmnopqrstuvwxyz','ABCDEFGHIJKLMNOPQRSTUVWXYZ'),'" + this.searchString.ToUpper() + "')]");
    ...
    private bool LoadSearchResultsInGridA(string sFilePath, bool bEncrypted, string attBookXmlPageIdElement, string attBookXmlPageNameElement, XmlDocument objBookXmlDoc)
    {
      try
      {
        ...
        {
          if (this.checkBoxExactMatch.Checked)
            xNodeList = xmlDocument.SelectNodes("//Part[@" + this.attPartNumber + "=\"" + this.searchString + "\"]");
          else
            xNodeList = xmlDocument.SelectNodes("//Part[contains(@" + this.attPartNumber + ",\"" + this.searchString + "\")]");
        }
        else if (this.checkBoxExactMatch.Checked)
          xNodeList = xmlDocument.SelectNodes("//Part[translate(@" + this.attPartNumber + ",'abcdefghijklmnopqrstuvwxyz','ABCDEFGHIJKLMNOPQRSTUVWXYZ')=\"" + this.searchString.ToUpper() + "\"]");
        else
          xNodeList = xmlDocument.SelectNodes("//Part[contains(translate(@" + this.attPartNumber + ",'abcdefghijklmnopqrstuvwxyz','ABCDEFGHIJKLMNOPQRSTUVWXYZ'),\"" + this.searchString.ToUpper() + "\")]");
    ...
    private void InitializeComponent()
    {
      ...
      this.Name = nameof (frmPartNumberSearch);
      this.ShowIcon = false;
      this.ShowInTaskbar = false;
      this.StartPosition = FormStartPosition.CenterScreen;
      this.Text = "Search";
      this.Load += new EventHandler(this.frmPartNumberSearch_Load);
      this.FormClosed += new FormClosedEventHandler(this.frmPartSearch_FormClosed);
      this.FormClosing += new FormClosingEventHandler(this.frmPartNumberSearch_FormClosing);
      this.KeyDown += new KeyEventHandler(this.frmPartNumberSearch_KeyDown);
