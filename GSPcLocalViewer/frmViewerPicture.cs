...
namespace GSPcLocalViewer
{
    public class frmViewerPicture : DockContent
    {
        ...
        private void objDjVuCtl_AuthFailed(object sender, EventArgs e)
        {
            ...
            {
                var str = new AES().DLLDecode(Program.iniServers[this.frmParent.ServerId].items["SETTINGS", "AUTHENTICATION"], "0123456789ABCDEF");
                if (str.Contains("|"))
                {
                    string name = str.Substring(0, str.IndexOf("|", StringComparison.Ordinal));
                    string password = str.Substring(str.IndexOf("|", StringComparison.Ordinal) + 1, str.Length - (str.IndexOf("|", StringComparison.Ordinal) + 1));
        ...
        private void ShowDJVU(bool bState, string sSource)
        {
            ...
            else if (bState)
            {
                ...
                this.objDjVuCtl.ExportImageAs(sSource.Replace(".djvu", ".jpg"), "jpg");
