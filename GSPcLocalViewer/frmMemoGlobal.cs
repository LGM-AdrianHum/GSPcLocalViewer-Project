...
// Type: GSPcLocalViewer.frmMemoGlobal
    ...
    public class frmMemoGlobal : Form
    {
        ...
        private void frmMemoGlobal_FormClosing(object sender, FormClosingEventArgs e)
        {
            if (this.lstExportedGlobalMemoPictures.Count <= 0)
                return;
            for (int index = 0; index < this.lstExportedGlobalMemoPictures.Count; ++index)
            {
                if (File.Exists(this.lstExportedGlobalMemoPictures[index]))
                    File.Delete(this.lstExportedGlobalMemoPictures[index]);
            }
        }
        ...
        private void tsbSave_Click(object sender, EventArgs e)
        {
            try
            {
                        ...
                        if (this.SaveMemosOnAllBook(this.searchString))
                        {
                            try
                            {
                                if (this.tsbAddTxtMemo.Checked)
                                {
                                    ...
                                    {
                                        int num = (int)MessageBox.Show(this.GetResource("(E-MGL-EM002) Failed to load specified object", "(E-MGL-EM002)_FAILED", ResourceType.POPUP_MESSAGE), this.frmParent.Text, MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
                                        return;
                                    }
                                }
                                else if (this.tsbAddRefMemo.Checked)
                                {
                                    ...
                                    {
                                        int num = (int)MessageBox.Show(this.GetResource("(E-MGL-EM003) Failed to load specified object", "(E-MGL-EM003)_FAILED", ResourceType.POPUP_MESSAGE), this.frmParent.Text, MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
                                        return;
                                    }
                                }
                                else if (this.tsbAddHypMemo.Checked)
                                {
                                    if (!this.txtHypMemoUrl.Text.Trim().Equals(string.Empty))
                                    {
                                        ...
                                        this.objDjVuCtlGlobalMemo.SRC = (string)null;
                                        this.pnlHypMemoPreview.AutoScrollMinSize = new Size(0, 0);
                                    }
                                    else
                                    {
                                        int num = (int)MessageBox.Show(this.GetResource("(E-MGL-EM004) Failed to load specified object", "(E-MGL-EM004)_FAILED", ResourceType.POPUP_MESSAGE), this.frmParent.Text, MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
                                        return;
                                    }
                                }
                                else if (this.tsbAddPrgMemo.Checked)
                                {
                                    ...
                                    {
                                        int num = (int)MessageBox.Show(this.GetResource("(E-MGL-EM005) Failed to load specified object", "(E-MGL-EM005)_FAILED", ResourceType.POPUP_MESSAGE), this.frmParent.Text, MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
                                        return;
                                    }
                                }
                ...
                else if (this.tsbAddRefMemo.Checked)
                {
                    ...
                    {
                        if (this.intMemoType == 2)
                            this.frmParent.objFrmTasks.AddMemoToTree("ref", this.txtRefMemoServerKey.Text + " " + this.txtRefMemoBookId.Text + " " + this.txtRefMemoOtherRef.Text, "GlobalMemo", "");
                        else
                            this.AddMemoToList("ref", this.txtRefMemoServerKey.Text + " " + this.txtRefMemoBookId.Text + " " + this.txtRefMemoOtherRef.Text);
                        this.txtRefMemoServerKey.Text = string.Empty;
                        this.txtRefMemoBookId.Text = string.Empty;
                ...
                else if (this.tsbAddPrgMemo.Checked)
                {
                    ...
                    {
                        if (this.intMemoType == 2)
                            this.frmParent.objFrmTasks.AddMemoToTree("prg", this.txtPrgMemoExePath.Text + "|" + this.txtPrgMemoCmdLine.Text, "GlobalMemo", "");
                        else
                            this.AddMemoToList("prg", this.txtPrgMemoExePath.Text + "|" + this.txtPrgMemoCmdLine.Text);
                        this.txtPrgMemoExePath.Text = string.Empty;
                        this.txtPrgMemoCmdLine.Text = string.Empty;
                ...
                else if (this.intMemoType == 2)
                {
                    ...
                    if (str.ToString().ToUpper() == "TEXT")
                        ...
                        this.frmParent.objFrmTasks.UpdateMemoToTree("ref", this.txtRefMemoServerKey.Text + " " + this.txtRefMemoBookId.Text + " " + this.txtRefMemoOtherRef.Text, "GlobalMemo", "");
                    ...
                    else
                    {
                        if (!(str.ToString().ToUpper() == "PROGRAM"))
                            return;
                        this.frmParent.objFrmTasks.UpdateMemoToTree("prg", this.txtPrgMemoExePath.Text + "|" + this.txtPrgMemoCmdLine.Text, "GlobalMemo", "");
                    }
                }
                else
                {
                        ...
                        try
                        {
                            if (!this.tsbAddTxtMemo.Checked && !this.tsbAddRefMemo.Checked && (!this.tsbAddHypMemo.Checked && !this.tsbAddPrgMemo.Checked))
                            {
                                if (this.intMemoType == 2)
                                {
                                    ...
                                    {
                                        string str2 = this.frmParent.objFrmTasks.tvMemoTypes.SelectedNode.Name.Split('|')[2];
                                        if (str2.ToString().ToUpper() == "TEXT")
                                            this.frmParent.objFrmTasks.UpdateMemoToTree("txt", this.rtbTxtMemo.Text, "GlobalMemo", "");
                                        else if (str2.ToString().ToUpper() == "REFERENCE")
                                            this.frmParent.objFrmTasks.UpdateMemoToTree("ref", this.txtRefMemoServerKey.Text + " " + this.txtRefMemoBookId.Text + " " + this.txtRefMemoOtherRef.Text, "GlobalMemo", "");
                                        else if (str2.ToString().ToUpper() == "HYPERLINK")
                                            this.frmParent.objFrmTasks.UpdateMemoToTree("hyp", this.txtHypMemoUrl.Text, "GlobalMemo", this.txtDescription.Text.ToString());
                                        else if (str2.ToString().ToUpper() == "PROGRAM")
                                            this.frmParent.objFrmTasks.UpdateMemoToTree("prg", this.txtPrgMemoExePath.Text + "|" + this.txtPrgMemoCmdLine.Text, "GlobalMemo", "");
                                    }
                                }
                                else if (this.dgMemoList.SelectedRows.Count > 0)
            ...
            if (this.intMemoType == 2)
            {
                try
                {
                    if (this.frmParent.objFrmTasks.tvMemoTypes.Nodes.Count <= 0 || this.frmParent.objFrmTasks.tvMemoTypes.SelectedNode.Parent == null)
                        return;
                    string key = this.frmParent.objFrmTasks.tvMemoTypes.SelectedNode.Name.Split('|')[4];
                    if (this.strMemoChangedType != string.Empty)
                    {
                        ...
                        this.strMemoChangedType = this.frmParent.objFrmTasks.tvMemoTypes.SelectedNode.Name.Split('|')[2];
                    this.dicGlobalMemoList.Remove(key);
                        ...
                        try
                        {
                            ...
                            try
                            {
                                ...
                                if (this.strDelMemoDate != string.Empty)
                                {
                                    ...
                                    this.strDelMemoDate = xmlNode.Attributes["Update"].Value.ToString();
                                empty1 = xmlNode.Attributes["Value"].Value.ToString();
                                empty2 = xmlNode.Attributes["Type"].Value.ToString();
                            }
                            catch (Exception ex)
                            {
                            }
                            string[] strArray = this.strDelMemoDate.Split('|');
                            int num = strArray.Length <= 0 ? this.GetEffectedMemoNodes("GLOBAL", this.frmParent.sServerKey, this.frmParent.sBookId, this.frmParent.sPartNumber, empty2, empty1, this.strDelMemoDate) : this.GetEffectedMemoNodes("GLOBAL", this.frmParent.sServerKey, this.frmParent.sBookId, this.frmParent.sPartNumber, empty2, empty1, strArray[strArray.Length - 1]);
        ...
        private void btnRefMemoOpen_Click(object sender, EventArgs e)
        {
            ...
            if (this.frmParent.objFrmMemoGlobal.isMemoChanged || this.frmParent.objFrmMemoLocal.isMemoChanged)
            {
                switch (MessageBox.Show(this.GetResource("(E-MGL-EM008) Specified information does not exist", "(E-MGL-EM008)_INFORMATION", ResourceType.POPUP_MESSAGE), this.frmParent.Text.Trim(), MessageBoxButtons.YesNoCancel, MessageBoxIcon.Question))
                {
                    ...
                    case "GIF":
                        if (!flag)
                        {
                            ...
                            if (this.txtHypMemoUrl.Text.ToUpper().EndsWith("DJVU"))
                            {
                                ...
                                {
                                    if (!(this.objDjVuCtlGlobalMemo.SRC != this.strPicFilePath))
                                        break;
                ...
                if (this.intMemoType == 2)
                {
                    try
                    {
                        if (this.frmParent.objFrmTasks.tvMemoTypes.Nodes["GlobalMemo"].Nodes.Count <= 0)
                            return (string[])null;
                        string[] strArray = !this.bSaveMemoOnBookLevel || !this.chkSaveBookLevelMemos.Checked || (!(this.frmParent.sPartNumber != string.Empty) || this.intMemoType == 2) ? new string[this.frmParent.objFrmTasks.tvMemoTypes.Nodes["GlobalMemo"].Nodes.Count] : new string[this.dicGlobalMemoList.Count];
                        int index = 0;
                        foreach (KeyValuePair<string, string> dicGlobalMemo in this.dicGlobalMemoList)
                        {
                            strArray[index] = dicGlobalMemo.Value.ToString();
                            ++index;
                        }
                        return strArray;
                    }
        ...
        private void AddMemoToList(string type, string value)
        {
            ...
            if (this.bSaveMemoOnBookLevel && this.chkSaveBookLevelMemos.Checked && (this.frmParent.sPartNumber != string.Empty && this.intMemoType != 2))
            {
                if (this.lstResults.Count > 1)
                {
                    ...
                    {
                        frmMemoGlobal frmMemoGlobal = this;
                        frmMemoGlobal.strMemoTagInfo = frmMemoGlobal.strMemoTagInfo + this.CreateMemoNode(type, value, str3).OuterXml + "^";
        ...
        private void UpdateMemoToList(DataGridViewRow row)
        {
            ...
            if (!str2.ToUpper().Equals("TXT") && !str2.ToUpper().Equals("HYP") && str4.Contains("|"))
                str4 = str4.Replace("|", " ");
            int updatedMemoNodes = this.GetUpdatedMemoNodes("GLOBAL", this.frmParent.sServerKey, this.frmParent.sBookId, this.frmParent.sPartNumber, str2, empty4, strDate);
            ...
            if (this.bSaveMemoOnBookLevel && this.chkSaveBookLevelMemos.Checked && (this.frmParent.sPartNumber != string.Empty && this.intMemoType != 2))
            {
                if (updatedMemoNodes > 1)
                {
                    ...
                    {
                        frmMemoGlobal frmMemoGlobal = this;
                        frmMemoGlobal.strMemoTagInfo = frmMemoGlobal.strMemoTagInfo + this.UpdateMemoNode(str2, attValue, str1).OuterXml + "^";
                        ++this.iMemoResultCounter;
                    }
        ...
        private string GetResource(string sDefaultValue, string sKey, ResourceType rType)
        {
            try
            {
                string str = "" + "/Screen[@Name='MEMO']" + "/Screen[@Name='GLOBAL_MEMO']";
                switch (rType)
                {
        ...
        private void ShowDJVU(bool bState, string sSource)
        {
            if (this.objDjVuCtlGlobalMemo.InvokeRequired)
                this.objDjVuCtlGlobalMemo.Invoke((Delegate)new frmMemoGlobal.ShowDJVUDelegate(this.ShowDJVU), (object)bState, (object)sSource);
            else if (bState)
            {
        ...
        private int GetEffectedMemoNodes(string strMemoFileType, string strServerKey, string strBookId, string strPartNo, string strType, string strValue, string strDate)
        {
            ...
            try
            {
                XmlDocument xmlDocument = new XmlDocument();
                string empty = string.Empty;
                string path1 = Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData) + "\\" + Application.ProductName + "\\" + Program.iniServers[this.frmParent.frmParent.p_ServerId].sIniKey;
                if (!Directory.Exists(path1))
                    Directory.CreateDirectory(path1);
                string str = !(strMemoFileType.Trim().ToUpper() == "GLOBAL") ? path1 + "\\LocalMemo.xml" : path1 + "\\GlobalMemo.xml";
                string path2 = string.Empty;
                if (strMemoFileType.ToUpper() == "GLOBAL")
                    path2 = Settings.Default.GlobalMemoFolder + "\\GlobalMemo.xml";
        ...
        private void InitializeComponent()
        {
            ...
            this.objDjVuCtlGlobalMemo.BeginInit();
            ...
            this.objDjVuCtlGlobalMemo.Size = new Size(406, 22);
            ...
            this.lblGlobalMemo.Size = new Size(448, 27);
            ...
            this.pnlForm.Controls.Add((Control)this.lblGlobalMemo);
            ...
            this.pnlHypMemoPreview.Controls.Add((Control)this.objDjVuCtlGlobalMemo);
            this.pnlHypMemoPreview.Dock = DockStyle.Fill;
        ...
        (ToolStripItem) this.tsbRefresh
            ...
            this.Name = nameof(frmMemoGlobal);
            this.Load += new EventHandler(this.frmMemoGlobal_Load);
            this.FormClosing += new FormClosingEventHandler(this.frmMemoGlobal_FormClosing);
            this.objDjVuCtlGlobalMemo.EndInit();
