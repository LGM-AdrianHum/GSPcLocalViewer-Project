...
// Type: GSPcLocalViewer.frmConfigSearchText
...
{
  public class frmConfigSearchText : Form
  {
    ...
    private void InitializeComponent()
    {
      ...
      this.Name = "frmConfigSearchPageName";
    ...
    public frmConfigSearchText(frmConfig frm, int _serverId)
    {
      ...
      try
      {
        ...
        if (Settings.Default.appLanguage.ToUpper() != "ENGLISH")
        {
          for (int index = 0; index < this.dgViewSearchSettings.Rows.Count; ++index)
            Global.SaveToLanguageIni("TEXT_SEARCH", this.dgViewSearchSettings[1, index].Value.ToString(), this.dgViewSearchSettings[2, index].Value.ToString(), this.serverId);
        }
        for (int index = 0; index < this.dgViewSearchSettings.Rows.Count; ++index)
        {
          string str;
          if (Settings.Default.appLanguage.ToUpper() != "ENGLISH")
            str = Global.GetEngHeaderVal("TEXT_SEARCH", this.dgViewSearchSettings[1, index].Value.ToString(), this.serverId).Split('|')[1];
          else
            str = this.dgViewSearchSettings[2, index].Value.ToString();
          Global.SaveToServerIni("TEXT_SEARCH", this.dgViewSearchSettings[1, index].Value.ToString(), this.dgViewSearchSettings[0, index].Value.ToString() + "|" + str + "|" + this.dgViewSearchSettings[4, index].Value.ToString() + "|" + this.dgViewSearchSettings[3, index].Value.ToString(), this.serverId);
        }
      }
      catch
      {
    ...
    private void InitializeSearchGrid()
    {
      ...
      {
        this.dgViewSearchSettings.Invoke((Delegate) new frmConfigSearchText.InitializeSearchGridDelegate(this.InitializeSearchGrid));
      }
      else
      {
        try
    ...
    private void AddSearchSettingsRow(string sVal, string sKey)
    {
      try
      {
        ...
        gridViewTextBoxCell2.Value = (object) Global.GetDGHeaderCellValue("TEXT_SEARCH", sKey, strArray1[1], this.serverId);
    ...
    private void LoadDataGridView()
    {
      if (this.dgViewSearchSettings.InvokeRequired)
      {
        this.dgViewSearchSettings.Invoke((Delegate) new frmConfigSearchText.LoadDataGridViewXmlDelegate(this.LoadDataGridView));
      }
      else
      {
        ...
        ArrayList keys = iniFileIo.GetKeys(Application.StartupPath + "\\GSP_" + Program.iniServers[this.serverId].sIniKey + ".ini", "TEXT_SEARCH");
        for (int index = 0; index < keys.Count; ++index)
        {
          try
          {
            this.AddSearchSettingsRow(new IniFileIO().GetKeyValue("TEXT_SEARCH", keys[index].ToString(), Application.StartupPath + "\\GSP_" + Program.iniServers[this.serverId].sIniKey + ".ini"), keys[index].ToString());
          }
          catch
          {
          }
        ...
        if (keys.Count != 0)
          return;
        this.AddSearchSettingsRow("true|Text|C|100", "Text");
        this.AddSearchSettingsRow("true|Page Name|C|100", "PageName");
        this.AddSearchSettingsRow("true|Pic Index|C|100", "PicIndex");
        this.AddSearchSettingsRow("true|Image Index|C|100", "ImageIndex");
      }
    }
    ...
    private void LoadResources()
    {
      ...
      try
      {
        this.dgViewSearchSettings.Columns["Display Name"].HeaderText = this.GetResource("Display Name", "DISPLAY_NAME", ResourceType.GRID_VIEW);
      }
      catch
      {
      }
      try
      {
        this.dgViewSearchSettings.Columns["Col.Width"].HeaderText = this.GetResource("Col Width", "COL_WIDTH", ResourceType.GRID_VIEW);
      }
      catch
      {
      }
    ...
    private string GetResource(string sDefaultValue, string sKey, ResourceType rType)
    {
      try
      {
        string str = "" + "/Screen[@Name='CONFIGURATION']" + "/Screen[@Name='TEXT_SEARCH']";
        switch (rType)
        {
