	...
	public class frmConfigSearchText : Form
	{
		...
		private void AddSearchSettingsRow(string sVal, string sKey)
		{
			try
			{
				...
				{
					Value = Global.GetDGHeaderCellValue("TEXT_SEARCH", sKey, strArrays[1], this.serverId)
				};
		...
		private void InitializeSearchGrid()
		{
			...
			{
				this.dgViewSearchSettings.Invoke(new frmConfigSearchText.InitializeSearchGridDelegate(this.InitializeSearchGrid));
				return;
			}
			try
			{
		...
		private void LoadDataGridView()
		{
			if (this.dgViewSearchSettings.InvokeRequired)
			{
				this.dgViewSearchSettings.Invoke(new frmConfigSearchText.LoadDataGridViewXmlDelegate(this.LoadDataGridView));
				return;
			}
			...
			arrayLists = iniFileIO.GetKeys(string.Concat(Application.StartupPath, "\\GSP_", Program.iniServers[this.serverId].sIniKey, ".ini"), "TEXT_SEARCH");
			for (int i = 0; i < arrayLists.Count; i++)
			{
				try
				{
					IniFileIO iniFileIO1 = new IniFileIO();
					string keyValue = iniFileIO1.GetKeyValue("TEXT_SEARCH", arrayLists[i].ToString(), string.Concat(Application.StartupPath, "\\GSP_", Program.iniServers[this.serverId].sIniKey, ".ini"));
		...
		public void SaveSettings()
		{
			try
			{
				...
				if (Settings.Default.appLanguage.ToUpper() != "ENGLISH")
				{
					...
					{
						Global.SaveToLanguageIni("TEXT_SEARCH", this.dgViewSearchSettings[1, i].Value.ToString(), this.dgViewSearchSettings[2, i].Value.ToString(), this.serverId);
					}
				}
				for (int j = 0; j < this.dgViewSearchSettings.Rows.Count; j++)
				{
					...
					{
						empty = Global.GetEngHeaderVal("TEXT_SEARCH", this.dgViewSearchSettings[1, j].Value.ToString(), this.serverId);
					...
					Global.SaveToServerIni("TEXT_SEARCH", str, string.Concat(strArrays), this.serverId);
				}
			}
			catch
			{
