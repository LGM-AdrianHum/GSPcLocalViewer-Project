	...
	public class frmMemoLocal : Form
	{
			...
			get
			{
				...
				if (this.intMemoType == 2)
				{
					try
					{
						...
						{
							str = (!this.frmParent.objFrmMemoLocal.bSaveMemoOnBookLevel || !this.frmParent.objFrmMemoLocal.chkSaveBookLevelMemos.Checked || !(this.frmParent.sPartNumber != string.Empty) || this.intMemoType == 2 ? new string[this.frmParent.objFrmTasks.tvMemoTypes.Nodes["LocalMemo"].Nodes.Count] : new string[this.dicLocalMemoList.Count]);
							int num = 0;
							foreach (KeyValuePair<string, string> keyValuePair in this.dicLocalMemoList)
							{
		...
		private void AddMemoToList(string type, string value)
		{
			...
			else
			{
				...
				{
					frmMemoLocal _frmMemoLocal = this;
					_frmMemoLocal.strMemoTagInfo = string.Concat(_frmMemoLocal.strMemoTagInfo, this.CreateMemoNode(type, value, empty1).OuterXml, "^");
					...
					else
					{
						...
						if (str != null)
						{
							...
							if (process1 != null)
							{
								...
								frmMemoLocal.SetForegroundWindow(process1.Handle);
							}
						}
						else
						{
							...
							if (process2 != null)
							{
								...
								frmMemoLocal.SetForegroundWindow(process2.Handle);
							}
						}
					}
				}
		...
		private void btnOK_Click(object sender, EventArgs e)
		{
			try
			{
				...
				else
				{
					...
					{
						System.Windows.Forms.DialogResult dialogResult = MessageBox.Show(this.GetResource("Do you want to save any changes made in memo", "SAVE_MEMO", ResourceType.POPUP_MESSAGE), this.frmParent.Text.Trim(), MessageBoxButtons.YesNoCancel, MessageBoxIcon.Question);
		...
		private void btnPrgMemoOpen_Click(object sender, EventArgs e)
		{
			...
			{
				MessageBox.Show(this.GetResource("(E-MLC-EM010) Can not open file/folder specified", "(E-MLC-EM010)_FOLDER", ResourceType.POPUP_MESSAGE), this.frmParent.Text.Trim(), MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
			}
		}
		...
		public void ClearItems(bool clearPanels, bool clearList, bool clearButtonCheck)
		{
			if (clearPanels)
			{
				...
				if (this.frmParent.objFrmTasks.intMemoType == 2)
				{
					...
					this.objDjVuCtlLocalMemo.SRC = null;
				}
			...
			try
			{
				if (xNode != null && xNode.Attributes.Count > 0)
				{
					foreach (DataGridViewRow row in (IEnumerable)this.dgMemoList.Rows)
					{
						...
						row.Tag = string.Concat(row.Tag, "^", xNode.OuterXml);
						flag = true;
						return flag;
					}
				}
		...
		private void frmMemoLocal_FormClosing(object sender, FormClosingEventArgs e)
		{
			...
			{
				for (int i = 0; i < this.lstExportedLocalMemoPictures.Count; i++)
				{
					if (File.Exists(this.lstExportedLocalMemoPictures[i]))
					{
						File.Delete(this.lstExportedLocalMemoPictures[i]);
					}
				}
			}
		}
		...
		private string GetResource(string sDefaultValue, string sKey, ResourceType rType)
		{
			...
			try
			{
				...
				str = string.Concat(str, "/Screen[@Name='LOCAL_MEMO']");
		...
		private void InitializeComponent()
		{
			...
			((ISupportInitialize)this.objDjVuCtlLocalMemo).BeginInit();
			...
			this.objDjVuCtlLocalMemo.Size = new System.Drawing.Size(406, 22);
			...
			this.lblLocalMemo.Padding = new System.Windows.Forms.Padding(3, 7, 0, 0);
			this.lblLocalMemo.Size = new System.Drawing.Size(448, 27);
			...
			this.pnlForm.Controls.Add(this.lblLocalMemo);
			...
			this.pnlHypMemoPreview.Controls.Add(this.objDjVuCtlLocalMemo);
			...
			base.Name = "frmMemoLocal";
			base.Load += new EventHandler(this.frmMemoLocal_Load);
			base.FormClosing += new FormClosingEventHandler(this.frmMemoLocal_FormClosing);
			((ISupportInitialize)this.objDjVuCtlLocalMemo).EndInit();
		...
		private void ShowDJVU(bool bState, string sSource)
		{
			...
			{
				AxDjVuCtrl axDjVuCtrl = this.objDjVuCtlLocalMemo;
				frmMemoLocal.ShowDJVUDelegate showDJVUDelegate = new frmMemoLocal.ShowDJVUDelegate(this.ShowDJVU);
		...
		private void tsbDelete_Click(object sender, EventArgs e)
		{
			...
			if (this.intMemoType == 2)
			{
				try
				{
					if (this.frmParent.objFrmTasks.tvMemoTypes.Nodes.Count > 0 && this.frmParent.objFrmTasks.tvMemoTypes.SelectedNode.Parent != null)
					{
						...
						char[] chrArray = new char[] { '|' };
						string str1 = name.Split(chrArray)[4];
						...
						else
						{
							...
							frmMemoLocal _frmMemoLocal1 = this;
							string str2 = _frmMemoLocal1.strMemoChangedType;
							string name2 = this.frmParent.objFrmTasks.tvMemoTypes.SelectedNode.Name;
							char[] chrArray2 = new char[] { '|' };
							_frmMemoLocal1.strMemoChangedType = string.Concat(str2, name2.Split(chrArray2)[2]);
						}
						if (this.bSaveMemoOnBookLevel && this.chkSaveBookLevelMemos.Checked && this.frmParent.sPartNumber != string.Empty && this.intMemoType != 2)
						{
							try
							{
								string item = this.dicLocalMemoList[str1];
								this.xDocDicMemos.LoadXml(item);
								XmlNode documentElement = this.xDocDicMemos.DocumentElement;
								this.smServerKey = this.frmParent.sServerKey;
								this.smBookID = this.frmParent.sBookId;
								this.smType = documentElement.Attributes["Type"].Value;
								this.smValue = documentElement.Attributes["Value"].Value;
								this.smPartNumber = this.frmParent.frmParent.objFrmPartlist.CurrentPartNumber;
								this.strDelMemoDate = documentElement.Attributes["Update"].Value;
								foreach (KeyValuePair<string, string> keyValuePair in this.dicLocalMemoList)
								{
									empty = keyValuePair.Key.ToString();
									str = keyValuePair.Value.ToString();
									this.xDocDicMemos.LoadXml(str);
									documentElement = this.xDocDicMemos.DocumentElement;
									if (!(documentElement.Attributes["ServerKey"].Value == this.smServerKey) || !(documentElement.Attributes["BookId"].Value == this.smBookID) || !(documentElement.Attributes["Type"].Value == this.smType) || !(documentElement.Attributes["Value"].Value == this.smValue) || !(documentElement.Attributes["PartNo"].Value == this.smPartNumber) || !(documentElement.Attributes["Update"].Value == this.strDelMemoDate))
									{
										continue;
									}
									this.dicLocalMemoList.Remove(empty);
								}
							}
							catch (Exception exception)
							{
						...
						this.dicLocalMemoList.Remove(str1);
			...
			else if (this.dgMemoList.SelectedRows.Count > 0)
			{
				if (!this.bSaveMemoOnBookLevel || !this.chkSaveBookLevelMemos.Checked || !(this.frmParent.sPartNumber != string.Empty) || this.intMemoType == 2)
				{
					...
					{
						frmMemoLocal _frmMemoLocal2 = this;
						_frmMemoLocal2.strMemoChangedType = string.Concat(_frmMemoLocal2.strMemoChangedType, "|");
						frmMemoLocal _frmMemoLocal3 = this;
						_frmMemoLocal3.strMemoChangedType = string.Concat(_frmMemoLocal3.strMemoChangedType, this.dgMemoList.SelectedRows[0].Cells[1].Value.ToString());
					}
					...
					try
					{
						...
						else
						{
							...
							string[] strArrays = str3.Trim().Split(new char[] { '\u005E' });
						...
						{
							frmMemoLocal _frmMemoLocal4 = this;
							_frmMemoLocal4.strDelMemoDate = string.Concat(_frmMemoLocal4.strDelMemoDate, "|");
							frmMemoLocal _frmMemoLocal5 = this;
							_frmMemoLocal5.strDelMemoDate = string.Concat(_frmMemoLocal5.strDelMemoDate, xmlNodes1.Attributes["Update"].Value.ToString());
						}
					}
				...
				else
				{
					try
					{
						...
						if (this.DeleteMemosOnAllBook(this.frmParent.sPartNumber))
						{
							...
							{
								frmMemoLocal _frmMemoLocal6 = this;
								_frmMemoLocal6.strMemoChangedType = string.Concat(_frmMemoLocal6.strMemoChangedType, "|");
								frmMemoLocal _frmMemoLocal7 = this;
								_frmMemoLocal7.strMemoChangedType = string.Concat(_frmMemoLocal7.strMemoChangedType, this.dgMemoList.SelectedRows[0].Cells[1].Value.ToString());
							}
							string empty1 = string.Empty;
							string empty2 = string.Empty;
							try
							{
								...
								string str4 = this.dgMemoList.SelectedRows[0].Tag.ToString();
								...
								else
								{
									...
									string[] strArrays1 = str4.Trim().Split(new char[] { '\u005E' });
								...
								{
									frmMemoLocal _frmMemoLocal8 = this;
									_frmMemoLocal8.strDelMemoDate = string.Concat(_frmMemoLocal8.strDelMemoDate, "|");
									frmMemoLocal _frmMemoLocal9 = this;
									_frmMemoLocal9.strDelMemoDate = string.Concat(_frmMemoLocal9.strDelMemoDate, xmlNodes.Attributes["Update"].Value.ToString());
								}
								empty1 = xmlNodes.Attributes["Value"].Value.ToString();
								empty2 = xmlNodes.Attributes["Type"].Value.ToString();
							}
							...
							int num = 0;
							num = ((int)strArrays2.Length <= 0 ? this.GetEffectedMemoNodes("LOCAL", this.frmParent.sServerKey, this.frmParent.sBookId, this.frmParent.sPartNumber, empty2, empty1, this.strDelMemoDate) : this.GetEffectedMemoNodes("LOCAL", this.frmParent.sServerKey, this.frmParent.sBookId, this.frmParent.sPartNumber, empty2, empty1, strArrays2[(int)strArrays2.Length - 1]));
							this.dgMemoList.Rows.Remove(this.dgMemoList.SelectedRows[0]);
							this.bMemoChanged = true;
			...
			try
			{
					...
					if (flag && this.SaveMemosOnAllBook(this.searchString))
					{
						try
						{
							if (this.tsbAddTxtMemo.Checked)
							{
								...
								{
									MessageBox.Show(this.GetResource("(E-MLC-EM002) Failed to save specified object", "(E-MLC-EM002)_FAILED", ResourceType.POPUP_MESSAGE), this.frmParent.Text, MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
									return;
								}
								else
								{
							...
							else if (this.tsbAddRefMemo.Checked)
							{
								if (this.txtRefMemoServerKey.Text.Trim().Equals(string.Empty) || this.txtRefMemoBookId.Text.Trim().Equals(string.Empty))
								{
									MessageBox.Show(this.GetResource("(E-MLC-EM003) Specified information does not exist", "(E-MLC-EM003)_INFORMATION", ResourceType.POPUP_MESSAGE), this.frmParent.Text, MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
									return;
								}
								else
								{
							...
							else if (this.tsbAddHypMemo.Checked)
							{
								...
								{
									MessageBox.Show(this.GetResource("(E-MLC-EM004) URL not found", "(E-MLC-EM004)_URL", ResourceType.POPUP_MESSAGE), this.frmParent.Text, MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
									return;
								}
								else
								{
							...
							else if (this.tsbAddPrgMemo.Checked)
							{
								...
								{
									MessageBox.Show(this.GetResource("(E-MLC-EM005) Specified information does not exist", "(E-MLC-EM005)_INFORMATION", ResourceType.POPUP_MESSAGE), this.frmParent.Text, MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
									return;
								}
								else
								{
						...
						{
							MessageBox.Show(this.GetResource("(E-MLC-EM006) Failed to save specified object", "(E-MLC-EM006)_SAVE", ResourceType.POPUP_MESSAGE), this.frmParent.Text, MessageBoxButtons.OK, MessageBoxIcon.Hand);
						}
				...
				else if (this.tsbAddRefMemo.Checked)
				{
					...
					else
					{
						...
						else
						{
							...
							frmMemoTask.AddMemoToTree("ref", string.Concat(text3), "LocalMemo", "");
						}
						this.txtRefMemoServerKey.Text = string.Empty;
						this.txtRefMemoBookId.Text = string.Empty;
						this.txtRefMemoOtherRef.Text = string.Empty;
				...
				else if (this.tsbAddPrgMemo.Checked)
				{
					...
					else
					{
						...
						{
							this.frmParent.objFrmTasks.AddMemoToTree("prg", string.Concat(this.txtPrgMemoExePath.Text, "|", this.txtPrgMemoCmdLine.Text), "LocalMemo", "");
						}
						this.txtPrgMemoExePath.Text = string.Empty;
						this.txtPrgMemoCmdLine.Text = string.Empty;
						this.lblHypMemoDate.Text = string.Empty;
				...
				else if (this.intMemoType == 2)
				{
					...
					else if (str4.ToString().ToUpper() == "REFERENCE")
					{
						...
						frmMemoTask1.UpdateMemoToTree("ref", string.Concat(strArrays3), "LocalMemo", "");
					}
					...
					{
						this.frmParent.objFrmTasks.UpdateMemoToTree("prg", string.Concat(this.txtPrgMemoExePath.Text, "|", this.txtPrgMemoCmdLine.Text), "LocalMemo", "");
					}
				}
				else if (this.dgMemoList.SelectedRows.Count > 0)
				{
				...
				else if (this.dgMemoList.Rows.Count > 0)
				{
					this.dgMemoList.SelectionChanged -= new EventHandler(this.dgMemoList_SelectionChanged);
				...
				try
				{
					if (this.tsbAddTxtMemo.Checked || this.tsbAddRefMemo.Checked || this.tsbAddHypMemo.Checked || this.tsbAddPrgMemo.Checked)
					{
						...
						if (!this.txtRefMemoServerKey.Text.Trim().Equals(string.Empty) && !this.txtRefMemoBookId.Text.Trim().Equals(string.Empty))
						{
							...
							else
							{
								frmMemoTasks frmMemoTask = this.frmParent.objFrmTasks;
								string[] strArrays = new string[] { this.txtRefMemoServerKey.Text, " ", this.txtRefMemoBookId.Text, " ", this.txtRefMemoOtherRef.Text };
								frmMemoTask.AddMemoToTree("ref", string.Concat(strArrays), "LocalMemo", "");
							}
						}
						if (!this.txtHypMemoUrl.Text.Trim().Equals(string.Empty))
						{
					...
					else if (this.intMemoType == 2)
					{
						try
						{
							string name = this.frmParent.objFrmTasks.tvMemoTypes.SelectedNode.Name;
							char[] chrArray = new char[] { '|' };
							string str = name.Split(chrArray)[2];
							if (this.frmParent.objFrmTasks.tvMemoTypes.SelectedNode.Parent.Name.ToString().ToUpper() == "LOCALMEMO")
							{
								...
								if (str.ToString().ToUpper() == "REFERENCE")
								{
									frmMemoTasks frmMemoTask1 = this.frmParent.objFrmTasks;
									string[] text1 = new string[] { this.txtRefMemoServerKey.Text, " ", this.txtRefMemoBookId.Text, " ", this.txtRefMemoOtherRef.Text };
									frmMemoTask1.UpdateMemoToTree("ref", string.Concat(text1), "LocalMemo", "");
								}
			...
			else
			{
				...
				try
				{
					...
					if (this.dgMemoList.Rows.Count <= 0)
					{
						...
						{
							string[] strArrays1 = new string[] { this.txtRefMemoServerKey.Text, " ", this.txtRefMemoBookId.Text, " ", this.txtRefMemoOtherRef.Text };
					...
					else
					{
						...
						else
						{
							...
							string[] strArrays2 = str2.Trim().Split(new char[] { '\u005E' });
							xmlTextReader = new XmlTextReader(new StringReader(strArrays2[0]));
							xmlNodes = xmlDocument.ReadNode(xmlTextReader);
						}
						empty1 = xmlNodes.Attributes["Type"].Value.ToString();
						str1 = xmlNodes.Attributes["Value"].Value.ToString();
						if (empty1 != string.Empty)
						{
							if (empty1.ToUpper() == "TXT")
							{
								empty = this.rtbTxtMemo.Text;
							}
							else if (empty1.ToUpper() == "REF")
							{
								string[] text3 = new string[] { this.txtRefMemoServerKey.Text, " ", this.txtRefMemoBookId.Text, " ", this.txtRefMemoOtherRef.Text };
								empty = string.Concat(text3);
							}
							else if (empty1.ToUpper() == "HYP")
							{
								empty = this.txtHypMemoUrl.Text;
							}
							else if (empty1.ToUpper() == "PRG")
							{
								empty = string.Concat(this.txtPrgMemoExePath.Text, "|", this.txtPrgMemoCmdLine.Text);
							}
						}
						...
						if (item.Tag.ToString().Contains<char>('\u005E'))
						{
							string str3 = item.Tag.ToString();
							char[] chrArray1 = new char[] { '\u005E' };
							string str4 = str3.Split(chrArray1)[0];
						}
					}
				}
				catch (Exception exception1)
				...
				if (empty != string.Empty && str1.Trim().ToUpper() == empty.Trim().ToUpper())
				{
					flag = false;
				}
				if (!flag)
					...
					Label0:
						...
						else
						{
							...
							else
							{
								...
								else if (this.objDjVuCtlLocalMemo.SRC != this.strPicFilePath)
								{
		...
		private void UpdateFont()
		{
			...
			this.lblLocalMemo.Font = new System.Drawing.Font(this.Font.FontFamily, this.Font.Size, FontStyle.Bold, this.Font.Unit, this.Font.GdiCharSet, this.Font.GdiVerticalFont);
				...
				if (!empty.Trim().Equals(string.Empty))
				{
					...
					if (!this.MatchXmlAttribute("Value", empty, empty2) || this.chkSaveBookLevelMemos.Checked)
					{
						...
						else
						{
							...
							_frmMemoLocal.strMemoChangedType = string.Concat(_frmMemoLocal.strMemoChangedType, "|");
							frmMemoLocal _frmMemoLocal1 = this;
							_frmMemoLocal1.strMemoChangedType = string.Concat(_frmMemoLocal1.strMemoChangedType, str2.ToUpper());
						}
						if (row.Tag.ToString().Contains<char>('\u005E'))
						{
						...
						try
						{
							...
							{
								frmMemoLocal _frmMemoLocal2 = this;
								_frmMemoLocal2.strDelMemoDate = string.Concat(_frmMemoLocal2.strDelMemoDate, "|");
								frmMemoLocal _frmMemoLocal3 = this;
								_frmMemoLocal3.strDelMemoDate = string.Concat(_frmMemoLocal3.strDelMemoDate, xmlNodes.Attributes["Update"].Value.ToString());
							}
						}
						catch (Exception exception)
						{
						...
						updatedMemoNodes = this.GetUpdatedMemoNodes("LOCAL", this.frmParent.sServerKey, this.frmParent.sBookId, this.frmParent.sPartNumber, empty1, empty4, empty3);
						...
						else
						{
							...
							{
								frmMemoLocal _frmMemoLocal4 = this;
								_frmMemoLocal4.strMemoTagInfo = string.Concat(_frmMemoLocal4.strMemoTagInfo, this.UpdateMemoNode(empty1, empty, str1).OuterXml, "^");
								this.iMemoResultCounter++;
							}
							this.dgMemoList.SelectedRows[0].Tag = string.Empty;
