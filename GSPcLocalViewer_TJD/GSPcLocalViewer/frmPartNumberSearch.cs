...
{
	public class frmPartNumberSearch : Form
	{
		...
		private string attPartNumberElement;
		...
		public frmPartNumberSearch(frmViewer frm, GSPcLocalViewer.frmSearch frmSearch)
		{
		...
		private void dgViewSearch_CellMouseDoubleClick(object sender, DataGridViewCellMouseEventArgs e)
		{
			try
			{
				if (this.dgViewSearch.Rows.Count != 0 && e.RowIndex >= 0)
				{
					...
					if (this.dgViewSearch["PartNumber", this.dgViewSearch.CurrentRow.Index].Value != null)
					{
		...
		private void frmPartNumberSearch_FormClosing(object sender, FormClosingEventArgs e)
		{
			Program.WriteSearchHistory(this.frmParent.sSearchHistoryPath, "PARTNUMBERSEARCH", Program.objPartNameSearchHistroyCollection);
		}

		private void frmPartNumberSearch_KeyDown(object sender, KeyEventArgs e)
		{
		...
		private void frmPartNumberSearch_Load(object sender, EventArgs e)
		{
			Program.ReadSearchHistory(this.frmParent.sSearchHistoryPath, "PARTNUMBERSEARCH", Program.objPartNumberSearchHistroyCollection);
			this.txtPartNumber.AutoCompleteCustomSource = Program.objPartNumberSearchHistroyCollection;
		...
		private string GetResource(string sDefaultValue, string sKey, ResourceType rType)
		{
			...
			try
			{
				...
				str = string.Concat(str, "/Screen[@Name='PART_NUMBER_SEARCH']");
		...
		private void GridViewAddRow(DataGridViewRow tempRow)
		{
			...
			frmPartNumberSearch.GridViewAddRowDelegate gridViewAddRowDelegate = new frmPartNumberSearch.GridViewAddRowDelegate(this.GridViewAddRow);
		...
		private void GridViewClearRows()
		{
			...
			this.dgViewSearch.Invoke(new frmPartNumberSearch.GridViewClearRowsDelegate(this.GridViewClearRows));
		}
		...
		private void HideLoading(Panel parentPanel)
		{
			try
			{
				...
				{
					frmPartNumberSearch.HideLoadingDelegate hideLoadingDelegate = new frmPartNumberSearch.HideLoadingDelegate(this.HideLoading);
		...
		private void InitializeComponent()
		{
			...
			this.lblPageName.Text = "Part Number";
			...
			this.checkBoxSearchWholeBook.TabIndex = 5;
			...
			this.txtPartNumber.TextChanged += new EventHandler(this.txtPartNumber_TextChanged);
			...
			base.Name = "frmPartNumberSearch";
			base.ShowIcon = false;
			base.ShowInTaskbar = false;
			base.StartPosition = FormStartPosition.CenterScreen;
			this.Text = "Search";
			base.Load += new EventHandler(this.frmPartNumberSearch_Load);
			base.FormClosed += new FormClosedEventHandler(this.frmPartSearch_FormClosed);
			base.FormClosing += new FormClosingEventHandler(this.frmPartNumberSearch_FormClosing);
			base.KeyDown += new KeyEventHandler(this.frmPartNumberSearch_KeyDown);
		...
		private void InitializeSearchGrid()
		{
			if (this.dgViewSearch.InvokeRequired)
			{
				this.dgViewSearch.Invoke(new frmPartNumberSearch.InitializeSearchGridDelegate(this.InitializeSearchGrid));
				return;
			}
			try
			{
		...
		private void LoadDataGridView()
		{
			if (this.dgViewSearch.InvokeRequired)
			{
				this.dgViewSearch.Invoke(new frmPartNumberSearch.LoadDataGridViewXmlDelegate(this.LoadDataGridView));
				return;
			}
			...
			arrayLists = iniFileIO.GetKeys(string.Concat(Application.StartupPath, "\\GSP_", Program.iniServers[this.frmParent.ServerId].sIniKey, ".ini"), "PARTNUMBER_SEARCH");
			for (int i = 0; i < arrayLists.Count; i++)
			{
				try
				{
					IniFileIO iniFileIO1 = new IniFileIO();
					string keyValue = iniFileIO1.GetKeyValue("PARTNUMBER_SEARCH", arrayLists[i].ToString(), string.Concat(Application.StartupPath, "\\GSP_", Program.iniServers[this.frmParent.ServerId].sIniKey, ".ini"));
					Global.AddSearchCol(keyValue, arrayLists[i].ToString(), "PARTNUMBER_SEARCH", this.frmParent.ServerId, ref this.dgViewSearch);
				}
				catch
				{
				}
			...
			if (arrayLists.Count == 0)
			{
				Global.AddSearchCol("true|Part Number|C|100", "PartNumber", "PARTNUMBER_SEARCH", this.frmParent.ServerId, ref this.dgViewSearch);
				Global.AddSearchCol("true|Part Name|C|100", "PartName", "PARTNUMBER_SEARCH", this.frmParent.ServerId, ref this.dgViewSearch);
				Global.AddSearchCol("true|Page Name|C|100", "PageName", "PARTNUMBER_SEARCH", this.frmParent.ServerId, ref this.dgViewSearch);
				Global.AddSearchCol("true|Page Id|C|100", "PageId", "PARTNUMBER_SEARCH", this.frmParent.ServerId, ref this.dgViewSearch);
			}
		}
		...
		{
			this.lblPageName.Text = this.GetResource("Part Number", "PART_NUMBER", ResourceType.LABEL);
			...
			try
			{
				...
				else
				{
					...
					if (xmlNodes != null)
					{
						...
						this.attPartNumberElement = string.Empty;
						foreach (XmlAttribute attribute in xmlNodes.Attributes)
						{
							if (!attribute.Value.ToUpper().Equals("ID"))
							{
								...
								this.attPartNumberElement = attribute.Name;
							}
							else
							{
						...
						if (this.attPartIdElement == "" || this.attPartNumberElement == "")
						{
							flag = false;
						}
						else
						{
							...
							if (this.checkBoxMatchCase.Checked)
							{
								if (this.checkBoxExactMatch.Checked)
								{
									...
									{
										string[] strArrays1 = new string[] { "//Part[@", this.attPartNumberElement, "='", this.searchString, "']" };
										xmlNodeLists = xmlDocument.SelectNodes(string.Concat(strArrays1));
									}
									else
									{
										string[] strArrays2 = new string[] { "//Part[@", this.attPartNumberElement, "='", strArrays[0], "' or @", this.attPartNumberElement, "='", strArrays[1], "']" };
										xmlNodeLists = xmlDocument.SelectNodes(string.Concat(strArrays2));
									}
								}
								else if (!hankakuZenkakuFlag || (int)strArrays.Length != 2)
								{
									string[] strArrays3 = new string[] { "//Part[contains(@", this.attPartNumberElement, ",'", this.searchString, "')]" };
									xmlNodeLists = xmlDocument.SelectNodes(string.Concat(strArrays3));
								}
								else
								{
									string[] strArrays4 = new string[] { "//Part[contains(@", this.attPartNumberElement, ",'", strArrays[0], "') or contains(@", this.attPartNumberElement, ",'", strArrays[1], "')]" };
							...
							else if (this.checkBoxExactMatch.Checked)
							{
								if (!hankakuZenkakuFlag || (int)strArrays.Length != 2)
								{
									string[] upper = new string[] { "//Part[translate(@", this.attPartNumberElement, ",'abcdefghijklmnopqrstuvwxyz','ABCDEFGHIJKLMNOPQRSTUVWXYZ')='", this.searchString.ToUpper(), "']" };
									xmlNodeLists = xmlDocument.SelectNodes(string.Concat(upper));
								}
								else
								{
									string[] upper1 = new string[] { "//Part[translate(@", this.attPartNumberElement, ",'abcdefghijklmnopqrstuvwxyz','ABCDEFGHIJKLMNOPQRSTUVWXYZ')='", strArrays[0].ToUpper(), "' or translate(@", this.attPartNumberElement, ",'abcdefghijklmnopqrstuvwxyz','ABCDEFGHIJKLMNOPQRSTUVWXYZ')='", strArrays[1].ToUpper(), "']" };
									xmlNodeLists = xmlDocument.SelectNodes(string.Concat(upper1));
								}
							}
							else if (!hankakuZenkakuFlag || (int)strArrays.Length != 2)
							{
								string[] upper2 = new string[] { "//Part[contains(translate(@", this.attPartNumberElement, ",'abcdefghijklmnopqrstuvwxyz','ABCDEFGHIJKLMNOPQRSTUVWXYZ'),'", this.searchString.ToUpper(), "')]" };
								xmlNodeLists = xmlDocument.SelectNodes(string.Concat(upper2));
							}
							else
							{
								string[] upper3 = new string[] { "//Part[contains(translate(@", this.attPartNumberElement, ",'abcdefghijklmnopqrstuvwxyz','ABCDEFGHIJKLMNOPQRSTUVWXYZ'),'", strArrays[0].ToUpper(), "') or contains(translate(@", this.attPartNumberElement, ",'abcdefghijklmnopqrstuvwxyz','ABCDEFGHIJKLMNOPQRSTUVWXYZ'),'", strArrays[1].ToUpper(), "')]" };
							...
							foreach (XmlNode xmlNodes1 in xmlNodeLists)
							{
								...
								for (int i = 0; i < this.dgViewSearch.Columns.Count; i++)
								{
									...
									foreach (XmlAttribute xmlAttribute in xmlNodes.Attributes)
									{
										...
										{
											if (!this.dgViewSearch.Columns[i].Name.ToUpper().Equals("PAGEID"))
											{
												continue;
											}
			...
			try
			{
				...
				else
				{
					...
					if (this.checkBoxMatchCase.Checked)
					{
						if (!this.checkBoxExactMatch.Checked)
						{
							string[] strArrays = new string[] { "//Part[contains(@", this.attPartNumber, ",\"", this.searchString, "\")]" };
							xmlNodeLists = xmlDocument.SelectNodes(string.Concat(strArrays));
						}
						else
						{
							string[] strArrays1 = new string[] { "//Part[@", this.attPartNumber, "=\"", this.searchString, "\"]" };
							xmlNodeLists = xmlDocument.SelectNodes(string.Concat(strArrays1));
						}
					}
					else if (!this.checkBoxExactMatch.Checked)
					{
						string[] upper = new string[] { "//Part[contains(translate(@", this.attPartNumber, ",'abcdefghijklmnopqrstuvwxyz','ABCDEFGHIJKLMNOPQRSTUVWXYZ'),\"", this.searchString.ToUpper(), "\")]" };
						xmlNodeLists = xmlDocument.SelectNodes(string.Concat(upper));
					}
					else
					{
						string[] upper1 = new string[] { "//Part[translate(@", this.attPartNumber, ",'abcdefghijklmnopqrstuvwxyz','ABCDEFGHIJKLMNOPQRSTUVWXYZ')=\"", this.searchString.ToUpper(), "\"]" };
						xmlNodeLists = xmlDocument.SelectNodes(string.Concat(upper1));
					}
					XmlNode xmlNodes = xmlDocument.SelectSingleNode("//Schema");
					xmlNodeLists = (new Filter(this.frmParent)).FilterPartsList(xmlNodes, xmlNodeLists);
		...
		private void ShowLoading(Panel parentPanel)
		{
			try
			{
				...
				{
					frmPartNumberSearch.ShowLoadingDelegate showLoadingDelegate = new frmPartNumberSearch.ShowLoadingDelegate(this.ShowLoading);
					object[] objArray = new object[] { this.pnlForm };
		...
		private void UpdadetControls()
		{
			try
			{
				...
				{
					base.Invoke(new frmPartNumberSearch.UpdadetControlsDelegate(this.UpdadetControls));
				}
			}
			catch
			{
